  ...
  public process (items) {
    return new Promise((resolve, reject) => {
      const rs = items.map((item) => {
        /*
        * this._req.query._source_exclude.indexOf('sgn') < 0 cause error "No additional data adapter for attribute"
        * possibly we do not need to sign output in default processor.
        * */
