...
export const actions: ActionTree<PaypalState, any> = {
  ...
  async syncCartTotals ({ getters }) {
    ...
    const shippingMethods = getters['getShippingMethods'] || [];

    console.log('cccccccccccccccccccccccccccccccccccccccccc')
    console.log(shippingDetails)
    console.log(shippingMethods)

    const selectedShippingMethod = shippingMethods.find(item => item.method_code === shippingDetails.shippingMethod)
    await rootStore.dispatch('cart/syncTotals', {
      methodsData: {
        ...
        method_code: selectedShippingMethod.method_code || shippingDetails.shippingMethod,
