...
export function beforeRegistration(config, store) {
  ...
  if (!isServer && config.hasOwnProperty('paypal') && config.paypal.addJsToGlobalHead
    && (window as any).paypalScriptLoaded === undefined) {
    ...
    (window as any).paypalScriptLoaded = true;
  }
}
